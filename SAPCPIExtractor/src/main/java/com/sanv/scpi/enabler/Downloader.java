...
public class Downloader {
  public static void downloadDeployedIflows(tenantConfiguration tenant) throws Exception {
    ...
    for (JsonElement iflowIDJE : iflowIDArr) {
      ...
      String packageName = "";
      System.out.format("IFLOW %03d/%03d : Downloading IFlow %s%n", new Object[] { Integer.valueOf(i + 1), Integer.valueOf(totalIflow), iflowName });
      ...
      apiUri = String.format("/api/v1/IntegrationDesigntimeArtifacts(Id='%s',Version='active')/$value", new Object[] { symbolicName });
      completeURL = "https://" + tenant.getTMNHost() + apiUri;
      HttpResponse iflowZip = Utility.doGet(completeURL, tenant);
      if (iflowZip.getStatusLine().getStatusCode() == 404) {
        System.out.format("IFLOW %03d/%03d : Integration design time artifact not found%n", new Object[] { Integer.valueOf(i + 1), Integer.valueOf(totalIflow) });
        continue;
      } 
      if (iflowZip.getStatusLine().getStatusCode() == 400) {
        System.out.format("IFLOW %03d/%03d : Cannot download the artifact from a configure only package.%n", new Object[] { Integer.valueOf(i + 1), Integer.valueOf(totalIflow) });
        continue;
      } 
      ZipInputStream zipIn = new ZipInputStream(iflowZip.getEntity().getContent());
      Utility.writeFile(zipIn, String.valueOf(tenant.getName()) + "/DeployedIFlow/" + packageName + "/" + symbolicName);
      i++;
    } 
  }
  ...
  public static void downloadAllPackage(tenantConfiguration tenant) throws Exception {
    ...
    for (scpiPackage pckJE : packgArr) {
      ...
      System.out.format("Package %02d/%d : Downloading Package - %s%n", new Object[] { Integer.valueOf(i + 1), Integer.valueOf(totalPackage), packageName });
      HttpResponse packageZip = fetchPackage(packageID, tenant);
      if (packageZip.getStatusLine().getStatusCode() == 404) {
        System.out.format("               Package %s Integration design time artifact not found.%n", new Object[] { packageName });
        i++;
        continue;
      } 
      if (packageZip.getStatusLine().getStatusCode() == 400) {
        System.out.format("               Package %s Cannot download - Configure only package.%n", new Object[] { packageName });
        i++;
        continue;
      } 
      if (packageZip.getStatusLine().getStatusCode() == 500) {
        System.out.format("                Downloading Failed  - %s Contains Artifacts in Draft Status.%n", new Object[] { packageName });
        i++;
        continue;
      } 

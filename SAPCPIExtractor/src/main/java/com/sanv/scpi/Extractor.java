...
//			trg = getTenant(to, tc);
            ...
            switch (mode) {
                ...
                case "configuration":
                    ...
                    break;
                case "valueMap":
					List<valueMap> vMaps = Design.getValueMapList(src);
					fileName = src.getId() + "_valueMap.json";
					file = new File(folderName, fileName);
					Utility.writeFile(file, vMaps);
					break;
    ...
    public static void verifyMode(String mode) throws Exception {
        ...
        allowedMode = Arrays.asList("designtime", "runtime", "configuration", "valueMap", "downloadAllPackage", "downloadPackage", "downloadDeployedIFlow", "getConnections", "downloadExtConfiguration");
